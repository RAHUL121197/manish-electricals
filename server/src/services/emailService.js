...
async function sendEnquiryEmail({ customerName, email, phone, subject, message }) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
      ...
      <p><strong>Mobile:</strong> ${phone || 'N/A'}</p>
      <p><strong>Subject:</strong> ${subject || 'N/A'}</p>
      <p><strong>Message:</strong></p>
      <div style="padding: 12px; background: #f8fafc; border-left: 4px solid #f59e0b; white-space: pre-wrap;">${message || 'N/A'}</div>
      <p style="margin-top: 16px;"><strong>Date & Time:</strong> ${now}</p>
    </div>
  `;
